...
**Objective Function**  
The energy system is given by the sum of capital, operation, and maintenance costs[^4]:
...
\[
J_c = \sum_{i \in \mathcal{M}} (\lambda_i S_i + \mu_i) \omega_i \tag{2}
\]

with

\[
\lambda_i = w_i \cdot \theta_i, \quad \mu_i = w_i \cdot \pi_i \tag{3}
\]

and constraints:

\[
w_i \in \{0,1\}^m, \quad S_i^{\text{min}} \leq S_i \leq S_i^{\text{max}} \tag{4}
\]

The **operation cost** \( J_o \) is given by:

\[
J_o = \sum_{j \in \mathcal{E}} \sum_{t=1}^T (u_j U_{j,t} - v_j V_{j,t}) \Delta t \tag{5}
\]

The **maintenance cost** \( J_m \) is calculated as a fraction of the capital cost:

\[
J_m = \sum_{i \in \mathcal{M}} \psi_i J_{c,i} \tag{6}
\]

Additionally, a **carbon emission constraint** is applied to ensure that total emissions \( e \) do not exceed a maximum threshold:

\[
e = \sum_{j \in \mathcal{E}} \epsilon_j \left( \sum_{t=1}^T U_{j,t} \right) \Delta t \leq e_{\text{max}} \tag{7}
\]
